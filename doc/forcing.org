...
and obvisously you must ensure that ~i*dt_forc=dt_forc_cycle~ with ~i~ an
integer. If you want a stationary forcing, simply put ~dt_forc=dt_forc_cycle~.

The forcing fields are read in a file named ~input_vars.nc~ which must be placed
in the same folder as the executable. This netCDF file must contain the a
variable named ~q_forcing_3d~ with ~i~ records and the same spatial dimensions
as a ~q~ fields that you would obtain in a regular output file.

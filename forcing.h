...
#define complex(i,j) f_idx(Nx - i, Ny - j)
#define real(i,j) f_idx(i,j)
...
/* Function to approximate inverse cumulative function of normal distribution. From https://web.archive.org/web/20150910081113/http://home.online.no/~pjacklam/notes/invnorm/impl/sprouse/ltqnorm.c */
...
void  init_det_forc(){
  ...
  for(int j = 0; j<Nyp1; j++){
    for(int i = 0; i <Nxp1; i++){
			forc[f_idx(i,j)] = (-tau0/Ly*pi*sin(pi*Y[j]/Ly));
    }
  }
}
